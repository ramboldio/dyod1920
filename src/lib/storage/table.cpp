...
void Table::compress_chunk(ChunkID chunk_id) {
    ...
    for(int idx=0; idx < this->get_chunk(chunk_id).column_count(); idx++){
        ColumnID column_id = static_cast<ColumnID>(idx);
        const std::shared_ptr<BaseSegment> seg = this->get_chunk(chunk_id).get_segment(column_id);
        const DictionarySegment dict_seg = DictionarySegment<AllTypeVariant>(seg);
        const std::shared_ptr<BaseSegment> dict = opossum::make_shared_by_data_type<opossum::BaseSegment, opossum::DictionarySegment>(
                column_type(column_id), _chunks[chunk_id].get_segment(column_id));
        compressed_chunk.add_segment(dict);
    }
    this->_chunks[chunk_id] = std::move(compressed_chunk);
